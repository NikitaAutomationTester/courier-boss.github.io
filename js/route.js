...
const SCROLL_ZONE_HEIGHT = 60; // Высота зоны прокрутки в пикселях
const SCROLL_SPEED = 8; // Скорость прокрутки
...
function handleTouchMove(e) {
  ...
  if (container) {
    ...
    if (touch.clientY < rect.top + SCROLL_ZONE_HEIGHT) {
      startScrolling(-SCROLL_SPEED);
    } else if (touch.clientY > rect.bottom - SCROLL_ZONE_HEIGHT) {
      startScrolling(SCROLL_SPEED);
    } else {
      stopScrolling();
    }
